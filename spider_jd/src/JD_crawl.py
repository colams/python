...
class JD_crawl:
    ...
    def login(self):
        ...
        try:
            ...
            #cache.store_session('JD_login', self.session)
            #login_page.content.decode('unicode_escape')
            cache.store_session('JD_login', pickle.dumps(self.session))
            print(login_page.text.encode('utf-8').decode('unicode_escape'))
        except Exception as e:
            print(e)

    def shopping(self):
        login = self.session.post('https://cart.jd.com/cart.action', headers=self.headers)
        #cache.store_session('JD_shoppinp', self.session)
        cache.store_session('JD_shoppinp', pickle.dumps(self.session))
        print(login.text)

    def shopping_v2(self,session):
